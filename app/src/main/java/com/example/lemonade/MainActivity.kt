...
class MainActivity : AppCompatActivity() {
    ...
    private fun setViewElements() {
        ...
        val imageLemonadeState: ImageView = findViewById(R.id.image_lemon_state)
        // TODO: set up a conditional that tracks the lemonadeState
        if (lemonadeState == SELECT) {
            textAction.text = getString(R.string.lemon_select)
            imageLemonadeState.setImageResource(R.drawable.lemon_tree)
        }

        if (lemonadeState == SQUEEZE) {
            textAction.text = getString(R.string.lemon_squeeze)
            imageLemonadeState.setImageResource(R.drawable.lemon_squeeze)
        }

        if (lemonadeState == DRINK) {
            textAction.text = getString(R.string.lemon_drink)
            imageLemonadeState.setImageResource(R.drawable.lemon_drink)
        }

        if (lemonadeState == RESTART) {
            textAction.text = getString(R.string.lemon_empty_glass)
            imageLemonadeState.setImageResource(R.drawable.lemon_restart)
        }
    }
